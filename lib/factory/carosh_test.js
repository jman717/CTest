...
module.exports = class carosh_test extends base{
	...
	insert_user(results){
		...
		try{
			var q = 'INSERT INTO popolana_ctest.users(first, last) values("' + g.req.body.first + '","' + g.req.body.last + '")';
			g.dbo.query(q, function(err, rows){
				var q2 = 'SELECT MAX(id) AS mx FROM popolana_ctest.users';
				g.dbo.query(q2, function(err, rows){
					var q3 = 'INSERT INTO popolana_ctest.user_to_role(user_id, role_id) values("' + rows[0].mx + '","1")';
					g.dbo.query(q3, function(err, rows){
						var q1 = 'SELECT * FROM popolana_ctest.users';
	...
	query_smith(results){
		...
		try{
			var q1 = 'SELECT * FROM popolana_ctest.users WHERE lower(last) = "smith"';
			g.dbo.query(q1, function(err, rows){
				g.res.json(rows);				
			});
			return results;
	...
	roles_assigned(results){
		...
		try{
			var q1 = 'SELECT ur.id, ur.first, ur.last, rl.role_name ' +
									'FROM popolana_ctest.users AS ur ' +
									'LEFT JOIN (popolana_ctest.user_to_role AS ru, popolana_ctest.roles AS rl) ' +
	...
	minimum(results){
		...
		try{
			var q1 = 'SELECT sb.count, sb.role_name FROM ( ' +
									'SELECT COUNT(DISTINCT(ru.user_id)) AS count, (SELECT rl.role_name FROM popolana_ctest.roles AS rl WHERE rl.id = ru.role_id) AS role_name ' +
									'FROM popolana_ctest.user_to_role AS ru ' +
									'WHERE ru.role_id > 0 ' +
									'GROUP BY ru.role_id ' +
									') AS sb ' +
									'WHERE sb.count >= 5 ';
