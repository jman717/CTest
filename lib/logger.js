...
exports = module.exports = class logger{
	constructor(callback) {
		...
		var app;
		log4js.configure({
			appenders: { ctest: { type: conf.vars.log_file_type, filename: conf.vars.log_file_name, daysToKeep: conf.vars.log_file_days_to_keep }},
		...
		app = t.tagline.appender('anyMsg');
		t.reqIP = new app().setConfig({"format": "reqIP(@ip)", "replace": "@ip"});
		app = t.tagline.appender('anyMsg');
		t.cli = new app().setConfig({"format": "client(@action)", "replace": "@action"});
		app = t.tagline.appender('anyMsg');
		t.qry = new app().setConfig({"format": "qry(@action)", "replace": "@action"});
		app = t.tagline.appender('anyMsg');
		t.classO = new app().setConfig({"format": "object(@class)", "replace": "@class"});
		app = t.tagline.appender('anyMsg');
		t.file = new app().setConfig({"format": "file(@file)", "replace": "@file"});
